...
static BOOL isNumber( PSZ_W pszText )
{
  ...
  while ( *pszText && fNumber )
  {
    ...
    if ( isdigit(chTest) || (chTest == L'.') || (chTest == L',') || (chTest == L'+') || (chTest == L'-')  )
    {
...
)
{
  ...
  usLen =  usTokenLen < usPropTokenLen? usTokenLen : usPropTokenLen ;
...
//+----------------------------------------------------------------------------+
//static 
void EQFBSimplifyAndCountMarks
(
