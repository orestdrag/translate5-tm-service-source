...
int OtmMorphICU::getTermType( UnicodeString text, int iStart, int iLen )
{
    ...
    for ( int i = 0; i < iLen; i++ )
    {
      ...
      if ( !isalnum(c) ) fAllAlNum = FALSE;
      if ( !isdigit(c) && (c != L',') && (c != L'.') && (c != L'/') && (c != L':') && (c != '-') ) fAllNumber  = FALSE;
      if ( !isalpha(c) || islower(c) ) fAllCaps = FALSE;
      if ( !isspace(c) ) fAllWhiteSpace = FALSE;
      if ( !ispunct(c) ) fAllPunctuation = FALSE;

      if ( isalpha(c) ) fContainsAlpha = TRUE;
      if ( isdigit(c) ) fContainsNumber = TRUE;
...
{
    LogMessage(T5FATAL,__func__, "::called TEMPORARY_COMMENTED OtmMorphICU::getResultFromIterator  whole function");
...
LogMessage(T5ERROR,__func__, ":: TEMPORARY_COMMENTED temcom_id = 73 // This code loads the rules from text file.    char line[MAX_RULE_LINE];");
#ifdef TEMPORARY_COMMENTED
...
int OtmMorphICU::checkTrailingSpaces(UnicodeString text, int start, int &end) {
    ...
    if ((start >= 1 && (text[start - 1] == NEWLINE_1 || text[start -1] == NEWLINE_2)) ||
        ...
        if ((tmp_index + 3) < end && blank_count >= 8 && isdigit(text[tmp_index]) && text[tmp_index + 1] == PERIOD && text[tmp_index + 2] == BLANK_SPACE && isalpha(text[tmp_index + 3])) {
            end = tmp_index + 3;
            return 1;
        }
        blank_count = 0;
    ...
    if ((index - 3) >= 0 && text[index] == BLANK_SPACE && text[index - 1] == PERIOD && isalpha(text[index - 2]) && isalpha(text[index - 3])) {
        int tmp_index = end;
        while (text[tmp_index] == BLANK_SPACE) {
            tmp_index++;
        }

        if (tmp_index && isdigit(text[tmp_index]) && text[tmp_index + 1] == PERIOD && text[tmp_index + 2] == BLANK_SPACE) {
            end -= newline_count;
            return return_value;
        }
    }
    ...
    if (!newline_count && blank_count && end == length && (text[index] == NEWLINE_1 || text[index] == NEWLINE_2)) {
        ...
        if ((tmp_index - 1) >= 0 && text[tmp_index] == PERIOD && isdigit(text[tmp_index - 1])) {
            return return_value;
        }
    }
	...
	if ( blank_count && index >= 0 && isalpha(text[index])) {
        return return_value;
    }
    ...
    if (((text[index] < 0x0041 || text[index] > 0x005A) && text[index] != COLON_MARK) && 
        ((newline_count && blank_count > MAX_TRAILING_SPACES_LENGTH_WITH_NEWLINE) ||
         (blank_count > MAX_TRAILING_SPACES_LENGTH_WITH_NO_NEWLINE && !isdigit(text[index])))) {
...
int OtmMorphICU::checkWordBoundary(UnicodeString text, int start, int &end) {
    ...
    start; newline_count;
    if ((index + 2) < length) {
        if (isalnum(text[index]) && IS_WORD_CONCATENATION_SYMBOL(text[index + 1]) && isalnum(text[index + 2])) {
            // alphanumbers-symbols-alphanumbers should be treated as one word
            end += 2;
        } else if (isdigit(text[index]) && IS_NUMBER_CONCATENATION_SYMBOL(text[index + 1]) && isdigit(text[index + 2])) {
...
void OtmMorphICU::listTerms( STRINGLIST& strResult, bool fUserAbbreviationList )
{
  ...
  int len = list.size();
  for (int i = 0; i < len; i++) {
    WideCharToMultiByte( CP_OEMCP, 0, (wchar_t*)list[i].c_str(), -1, tWord, 1024, NULL, NULL );
...
void OtmMorphICU::saveAbbreviations()
{
  ...
  FILE* tFilePointer;
    ...
    for (int i = 0; i < len; i++) {
      WideCharToMultiByte( CP_UTF8, 0, (wchar_t*)userAbbrevList[i].c_str(), -1, tWord, 1024, NULL, NULL );
