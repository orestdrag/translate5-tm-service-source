    ...
    *pIda->pErrorInfo = 0L;
    ...
    do {
      ...
      if( usrc || (sizread != sizeof( prophead))){
        T5LOG(T5ERROR) << "LoadPropFile, couldn't read file: " << fname <<  ", bytes read: " <<sizread;
      ...
      *pcntl->pHead = prophead;
      ...
      {
        T5LOG(T5ERROR) << "UtlRead:: usrc = "<<usrc << "; sizread = " << sizread << ", size = " << size;
        ...
        {
          T5LOG(T5ERROR) <<  ":: TEMPORARY_COMMENTED :: Err_ReadFile // files created in linux don't have that filler to fit 2k";
...
USHORT GetSysProp( PPROP_IDA pIda)
{
    ...
    do {
      T5LOG( T5DEBUG) << "GetSysProp::path = " << pIda->IdaHead.pszObjName;
...
VOID NotifyAll( HPROP hprop)
{
    ...
    if ( UtlQueryUShort( QS_RUNMODE ) != FUNCCALL_RUNMODE )
    {
      ...
      T5LOG(T5ERROR) <<  ":: TEMPORARY_COMMENTED temcom_id = 98 strcat( strcat( strcpy( name, ph->szPath), \"\\\"), ph->szName);";
#ifdef TEMPORARY_COMMENTED
                               ...
                               PEQFINFO pErrorInfo)
{
   ...
   {
     if(T5Logger::GetInstance()->CheckLogLevel(T5DEBUG)) T5LOG( T5WARNING) <<"if(true) hardcoded in OpenProperties";
     ...
     {
       T5LOG(T5ERROR) <<  "::TEMPORARY_COMMENTED call EqfCallPropertyHandler ";
#ifdef TEMPORARY_COMMENTED
...
)
{
   ...
   do {
     ...
     if(T5Logger::GetInstance()->CheckLogLevel(T5DEBUG))  T5LOG( T5WARNING) << "if(true) hardcoded in CloseProperties";
     if (true || UtlQueryUShort( QS_RUNMODE ) == FUNCCALL_RUNMODE )
     {
        PPROP_IDA     pIda;           // Points to instance area
        PPROPCNTL     pcntl;
     ...
     {
       T5LOG( T5WARNING) <<"TEMPORARY_COMMENTED in Close Properties, SHORT1FROMMR";
...
)
{
   ...
   do {
     ...
     {
T5LOG(T5ERROR) <<  ":: TEMPORARY_COMMENTED temcom_id = 100 rc = SHORT1FROMMR( EqfCallPropertyHandler( WM_EQF_SAVEPROPERTIES, MP1FROMSHORT(0), MP2FROMP(&PropMsg) ) );";
#ifdef TEMPORARY_COMMENTED
...
{
    HPROP hSysProp;
    //if(T5Logger::GetInstance()->CheckLogLevel(T5DEBUG))  T5LOG( T5WARNING) << "if(true) hardcoded in GetSystemPropPtr");
    if (true || UtlQueryUShort( QS_RUNMODE ) == FUNCCALL_RUNMODE )
    {
      PPROP_IDA     pIda;           // Points to instance area
      pIda = pPropBatchIda;
    ...
    else
    {
      ...
      T5LOG(T5ERROR) <<  ":: TEMPORARY_COMMENTED temcom_id = 101 assert( hSysProp != NULL );";
#ifdef TEMPORARY_COMMENTED
...
SHORT GetPropSize( USHORT usClass)
{
   ...
   switch ( usClass )
   {
      ...
      case PROP_CLASS_MEMORY_LASTUSED :
        if(T5Logger::GetInstance()->CheckLogLevel(T5DEBUG)) T5LOG( T5WARNING) << "if(true) hardcoded in GetPropSize::PROP_CLASS_MEMORY";
         if (true || UtlQueryUShort( QS_RUNMODE ) == FUNCCALL_RUNMODE )
         {
...
HPROP EqfQuerySystemPropHnd( void )
{
  ...
  T5LOG( T5WARNING) << "hardcoded if(true) in EqfQuerySystemPropHnd";
  if (true || UtlQueryUShort( QS_RUNMODE ) == FUNCCALL_RUNMODE )
  {
    ...
    if(pIda){
      ...
      hprop = NULL;
      T5LOG(T5ERROR) << "pId and pPropBatchIda are NULL";
    }
  }
  else
  {
